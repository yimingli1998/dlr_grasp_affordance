...
def test(output_path = 'output',split = 'test_easy'):
    ...
    for i, (data,index) in enumerate(tqdm(test_dataloader)):
        ...
        # t1 = time.time()
        # print(t1-t0
        for point,sem,index,gp,pose,joint in zip(bat_point,bat_sem,bat_index,bat_pred_graspable,bat_pred_pose,bat_pred_joint):
            ...
            for t in range(gp.size(-1)): # for each taxonomy
                tax = taxonomy[t]
                scene = trimesh.Scene()
                scene_mesh,_,_ = scene_utils.load_scene(index,split='test')
                ...
                if torch.sum(out_gp)>0:
                    ...
                    # print(type(out_sem))
                    np.save(os.path.join(output_path,f'img_{index}_quat.npy'),output_hand_grasp)
                    # break
...
def evaluate(path,img_id):
    ...
    for k in grasp.keys():
            ...
            if c> 0.1: # 0 is background
                ...
                if len(ins_pos) >0:
                    ...
                    if len(ins_pos) > 2: # each objec choice two grasp
                        ins_pos,ins_R,ins_joint = ins_pos[:2],ins_R[:2],ins_joint[:2]
        ...
        print(penetration)
    np.save(f'output_res/{img_id}_res_tax.npy',penetration)
        # np.save()
    # env.close()
    return penetration
...
if __name__ == '__main__':
    ...
    #     evaluate('output',i)
    # evaluate('output',799)
    # output = parallel_evaluate('output',split = 'test_medium',proc = 8)

    # # evaluate without tax
    # import glob
    # # # res = glob.glob('output_res/*_quat.npy')
    # res = glob.glob('output_res/*_res.npy')
    ...
    import glob
    # # res = glob.glob('output_res/*_quat.npy')
    res = glob.glob('output_res/*_tax.npy')
    output = [np.load(r,allow_pickle =True).item() for r in res]
    taxonomy = ['Parallel_Extension','Pen_Pinch','Palmar_Pinch','Precision_Sphere','Large_Wrap']
