...
def match_point_grasp(img_id,save_path):
    scene = trimesh.Scene()
    grasp_path = 'scene_grasp_affordance'
    taxonomies = grasp_dict_20f.keys()
    scene_id = img_id//cfg['num_images_per_scene']
    point,_ = scene_utils.load_scene_pointcloud(img_id, use_base_coordinate=cfg['use_base_coordinate'])
    grasp = np.load(
...
if  __name__ =='__main__':
    save_path = 'point_grasp_data_with_affordance'
    if os.path.exists(save_path) is False:
        os.makedirs(save_path)
    # for i in range(100):
    #     match_point_grasp(i,save_path)
    parallel_match_point_grasp(16,save_path)
